...
class LogisticalRegression(Classification):
    ...
    def run_classifier(self, data):
        ...
        #fitting for main classifier done here
        ...
        #apply training and test set split
        self.train_test_split(data=data, features=lagged_headers, size=0.6, test_param=returns_sign)
    ...
    def train_test_split(self, data=None, features=None, size=None, test_param=None):
        ...
        self._logger.info("Finished test train with split: {}".format(size))
    
    def compute_confusion_matrix(self, data):
        self.fit_model(data.x_train, data.y_train, self.logreg_2)
        y_pred = self.logreg_2.predict(data.x_test)
        print("Confusion matrix as follows:")
        data.c_matrix = confusion_matrix(data.y_test, y_pred)
        print(data.c_matrix)
    ...
    def compute_population_data_roc_metrics(self, data, y_population):
        ...
        self._logger.info("ROC AUC Population Data Score: {}".format(roc_auc_score(y_population, data.y_pred_prob_population)))
